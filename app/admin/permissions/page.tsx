...
function AccessCell({ access }: { access: Access }) {
  ...
  if (access === 'none') return <X className="mx-auto h-4 w-4 text-gray-300" />
  if (access === 'own')
...
export default async function PermissionsPage() {
  ...
  return (
    <div>
      <h1 className="mb-1 text-xl font-bold text-gray-900">権限マトリクス</h1>
      ...
      <div className="overflow-hidden rounded-xl border border-gray-200 bg-white">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-100">
            {FEATURES.map((f) => (
              <tr key={f}>
                <td className="px-4 py-3 font-medium text-gray-900">{FEATURE_LABEL[f]}</td>
                {ROLES.map((r) => (
      ...
      <div className="mt-4 flex flex-wrap gap-4 text-xs text-gray-500">
        ...
        <span className="flex items-center gap-1"><X className="h-3.5 w-3.5 text-gray-300" /> アクセス不可</span>
      </div>
    </div>
  )
}
