...
const field = 'flex-1 rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-brand-400 focus:outline-none'
...
export default async function DealNotesPage({
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <Link href={`/admin/crm/${id}`} className="mb-4 inline-flex items-center gap-1 text-sm text-gray-500 hover:text-gray-900">
        <ArrowLeft className="h-4 w-4" />
        顧客ページへ
      </Link>
      <h1 className="mb-6 text-xl font-bold text-gray-900">商談の対応履歴</h1>
      ...
      <ul className="space-y-3">
        ...
        {notes.map((n) => (
          <li key={n.id} className="rounded-lg border-l-2 border-brand-200 bg-white px-4 py-3 text-sm">
            ...
            <div className="mt-1 text-xs text-gray-400">{new Date(n.created_at).toLocaleString('ja-JP')}</div>
          </li>
        ))}
      </ul>
    </div>
